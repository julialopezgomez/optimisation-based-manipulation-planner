...
A section-by-section guide to `summer-work/nhr.py`, in the same order as the file, meant to be read side-by-side with the source. For *how to run it*, see the separate user guide; this doc is about *what each piece does and why it's shaped that way*.
