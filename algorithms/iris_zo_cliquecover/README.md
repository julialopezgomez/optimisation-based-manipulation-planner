...
Reimplementation of Drake's IRIS-ZO and clique-cover algorithms with an NLP-sampling twist for drawing polytopes, to be integrated with `algorithms/nhr.py` into the full-arm pipeline.

Not started yet - this package is an empty placeholder so the eventual code has a home from day one, mirroring how `algorithms/nhr.py` is structured (flat module(s), importable via `algorithms/` on `sys.path`, no packaging ceremony).
