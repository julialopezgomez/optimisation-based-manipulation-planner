...
- **Hardcoded absolute URDF URLs**: commented-out lines in `full_arm_nhr.ipynb` and `data/generation/full_arm_c_free.ipynb` reference `file:///home/julialopezgomez/optimisation-based-manipulation-planner/my_sdfs/panda_arm_locked.urdf` (and hand) - fragile/non-portable across machines or users, and now stale (that filename no longer exists; the current locked-arm/hand URDFs are `my_sdfs/panda_arm_locked_1dof.urdf`/`my_sdfs/panda_hand_locked_1dof.urdf`). Left alone since the lines are dead/commented out.
