"""
Fast numeric evaluator for #79's TC-space rational constraint - same
scene, same physical obstacle, same rational-polynomial constraint as
tc_space_collision_scenario_rational.py, but evaluated without Drake's
per-call symbolic Polynomial/RationalFunction.Evaluate (which walks a full
expression tree every call - confirmed to be the actual bottleneck behind
the ~230-290x slowdown reported in #46, not anything intrinsic to
rational algebra: see the benchmark below).
...
q_star) - so ~99% of the monomial terms have a nonzero power on a fixed
variable and evaluate to exactly zero. Extracting monomial_to_coefficient_map()
ONCE, dropping every term with nonzero power on a fixed variable, and
keeping only (power on s_j2, power on s_j4, coefficient) leaves ~225 terms
for the numerator and ~64 for the denominator (down from ~36804 and
~3072) - small enough to evaluate as plain vectorized numpy arithmetic,
no symbolic tree-walking per call. Verified to match Drake's own
evaluation to float64 precision (1.1e-16) before trusting it for
anything - see _benchmark_and_validate() at the bottom.
...
def _extract_2var_terms(poly, name_a, name_b):
    ...
    fixed at 0. Variable equality can't go through plain dict.get() (see
    module notes below) - Drake's Variable.__eq__ returns a symbolic
    Formula, not a bool, so matching is done by name string instead."""
...
def make_fast_rational_collision_constraint(scene: dict):
    """
    Same signature/contract as tc_space_collision_scenario_rational.
    make_rational_collision_constraint (g, Jg, lower, upper on s-space
    coordinates x=(s_j2, s_j4)), but g/Jg are plain numpy closures built
    from the extracted term lists, not Drake symbolic evaluation.
    """
...
def _benchmark_and_validate(num_trials_correctness=20, num_trials_timing=500):
    ...
    assert max_g_err < 1e-8 and max_jg_err < 1e-6, "fast evaluator doesn't match Drake's symbolic one - do not trust it"
