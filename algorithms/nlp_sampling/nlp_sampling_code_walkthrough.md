...
- `save_run_analytics_artifacts(samples, restart_info, lower, upper, joint_names, options, output_root, note, timestamp, bins, show, elapsed)` (line 1591) — the one-call preset for everything else a run reports: calls `save_joint_sample_artifacts` for the per-joint distributions, then plots the pooled reason-code histogram and a per-joint grid of `per_restart_spread` (within-chain mean ± std vs. restart), and writes a `run_analytics_summary.json` with the cleanup-ran fraction, accept rate, Phase-1/success counts, a final-residual summary (from each restart's last diagnostic step's `err`), and (if `elapsed` is passed) wall-clock/samples-per-second. This is what `nlp_sampling_standalone_test.py`'s `run_diagnostics` calls instead of reimplementing its own plotting.

## One-call orchestration (line 1752)

- `run_and_report(g, lower, upper, Jg, h, Jh, nhr_options, restart_options, joint_names, output_root, note, timestamp, bins, show, equality_tol, project_samples_to_manifold, projection_iters)` — the single entry point for the full "sample -> diagnose -> save" pipeline: wires a `run_downhill_phase1`-based `phase1` callable around `restarting_nhr_sample`, times the run, then hands the result straight to `save_run_analytics_artifacts`. Only needs `g`/`Jg`/`h`/`Jh` - no IK/Drake dependency - so it's usable for any constrained-sampling problem, not just ones with a Drake plant already built. Returns `(samples, restart_info, run_dir)`. `nlp_sampling_standalone_test.py`'s `main()` and `full_arm_nhr.ipynb` both call this instead of hand-assembling `phase1` + `restarting_nhr_sample` + `save_run_analytics_artifacts` themselves.
