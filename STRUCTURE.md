...
- **`manipulation_planner/`** - the full-arm/YAML-region `ManipulationPlanner` variant (grasp/placement polytopes -> MIP grasp-path -> GCS trajectory, C-free regions passed in or generated via IRIS-ZO/clique-cover), shared by `full_arm_nhr.ipynb` and `data/generation/full_arm_c_free.ipynb` - deduplicated from those notebooks' near-identical inline copies (issue #58). The per-instance Mosek-license-file check that used to live in `__init__` was dropped rather than moved; a single check is meant to live at session/notebook startup instead (not yet added). `_compute_connected_components` and `display_trajectory` still read `visualizer`/`panda_hand`/`panda_arm`/`cap` as module globals rather than instance state - an existing quirk carried over unchanged from the inline copies - so importing notebooks must set `manipulation_planner.manipulation_planner.{visualizer,panda_hand,panda_arm,cap}` before calling either method (both notebooks do this immediately after the import, matching where the inline class used to be defined).
...
- **`ManipulationPlanner` is copy-pasted**, not shared: at least 5 near-identical inline definitions existed (`manipulation_planner_3dof.ipynb`, `manipulation_planner_4dof.ipynb`, `full_arm_blocked_joints_3dof.ipynb`, `full_arm_blocked_joints_4dof.ipynb`, `data/generation/full_arm_c_free.ipynb`, `full_arm_nhr.ipynb`), with real drift between copies (e.g. only some perform a Mosek-license-file validity check in `__init__`). The full-arm/YAML-region variant (`data/generation/full_arm_c_free.ipynb` and `full_arm_nhr.ipynb` - these two differed by exactly one line) has been deduplicated into `algorithms/manipulation_planner/` (issue #58); the 3dof/4dof/blocked-joints copies are separate variants and remain inline, worth deduplicating once there's a stable interface to converge on.
